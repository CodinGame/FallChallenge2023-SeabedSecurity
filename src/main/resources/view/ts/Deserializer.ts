...
import { DroneDto, FishDto, FishGlobalDto, FrameDataDTO, GlobalData, GlobalDataDTO, PlayerDto, ScanSummaryDto, UglyDto } from './types.js'
...
export function parseData (unsplit: string, globalData: GlobalData): FrameDataDTO {
  ...
  const fishes: FishDto[] = []
  const fishMap: Record<number, FishDto> = {}
  ...
  const uglies: UglyDto[] = []
  const uglyMap: Record<number, UglyDto> = {}
  ...
  for (let i = 0; i < globalData.playerCount; ++i) {
    ...
    const playerScans: ScanSummaryDto[][] = []
    for (let row = 0; row < 4; ++row) {
      const line: ScanSummaryDto[] = []
...
function getComboCode (line: number[]) {
...
export function parseGlobalData (unsplit: string): GlobalDataDTO {
  ...
  const fishes: FishGlobalDto[] = []
  const fishMap: Record<number, FishGlobalDto> = {}

  for (let i = 0; i < fishCount; ++i) {
    const rawFish = splitLine(raw[idx++])
    const fish: FishGlobalDto = {
      id: +rawFish[0],
      color: +rawFish[1],
      type: +rawFish[2]
    }
