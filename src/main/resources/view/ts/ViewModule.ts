...
import { AnimData, CanvasInfo, ContainerConsumer, Drone, Effect, Entity, EntityDto, Fish, FishDto, FrameData, FrameInfo, GlobalData, PlayerInfo, ScanDto, SquidGrid, SquidGridCell, Ugly, UglyDto } from './types.js'
...
export class ViewModule {
  ...
  uglies: Ugly[]
  drones: Drone[][]
  sideViewEntities: Entity[]
  squidGrids: SquidGrid[]
  scoreTexts: PIXI.Text[]

  canvasMode: boolean
  discs: PIXI.Container[][]
  hidingUglyLayer: PIXI.Container
  movingUglyLayer: PIXI.Container
  ...
  convertPos ({ x, y }): {x: number, y: number} {
    return {
  ...
  updateEntity (prev: EntityDto, cur: EntityDto, entity: Entity) {
  ...
  updateScanLights (fishesScannedThisTurn: number[], drone: Drone, startP: number, endP: number) {
    fishesScannedThisTurn.forEach((fishId) => {
      ...
      if (fish) {
        ...
        const fx = this.getFromPool('beam').display as PIXI.AnimatedSprite
  ...
  setScanAnimationProgress (drone: Drone, progress: number) {
  ...
  updateUglies () {
    for (const ugly of this.uglies) {
      const prev: UglyDto = this.previousData.uglyMap[ugly.id]
      const cur: UglyDto = this.currentData.uglyMap[ugly.id]
      ...
      const animationSprite = foundTarget ? ugly.aggroSprite as PIXI.AnimatedSprite : ugly.sprite as PIXI.AnimatedSprite
  ...
  updateFishes () {
    for (const fish of this.fishes) {
      const prev: FishDto = this.previousData.fishMap[fish.id]
      const cur: FishDto = this.currentData.fishMap[fish.id]
      ...
      if (cur && prev) {
        ...
        fish.sprite.alpha = 1
        const spriteAnimation = fish.sprite as PIXI.AnimatedSprite
        ...
        fish.container.position.copyFrom(
          this.convertPos({
  ...
  shakeFish (entity: PIXI.DisplayObject) {
  ...
  animateScene (delta) {
    ...
    for (const fish of this.fishes) {
      const curFleeing = this.currentData.fishMap[fish.id]?.isFleeing ?? false
      const prevFleeing = this.previousData.fishMap[fish.id]?.isFleeing ?? false
      const showFleeing = (curFleeing && this.progress === 1) || (prevFleeing && this.progress < 1)
      if (showFleeing) {
        this.shakeFish(fish.container)
        const animationSprite = fish.sprite as PIXI.AnimatedSprite
  ...
  initSquidGrid (player: PlayerInfo): SquidGrid {
    const layer = new PIXI.Container()
    const GRID_RECT = {
    ...
    const array: SquidGridCell[][] = []
    for (let colorIdx = 0; colorIdx < 5; ++colorIdx) {
      const line: SquidGridCell[] = []
  ...
  initWalls (layer: PIXI.Container) {
  ...
  initFilter (container, background) {
    const flowmap = PIXI.TilingSprite.from('flowmap.png', { width: WIDTH, height: HEIGHT }) as (PIXI.TilingSprite)
  ...
  registerTooltip (container: PIXI.Container, getString: () => string, getScans?: () => ScanDto[]) {
    container.interactive = true
