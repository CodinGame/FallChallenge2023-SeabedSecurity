...
export class EndScreenModule {
  ...
  createEndScene () {
    ...
    background.drawRect(360, 0, WIDTH - 360 * 2, HEIGHT)
    background.endFill()
    ...
    if (this.finishers.length <= 4) {
      const maxFinisherWidth = Math.max(...this.finishers.map(f => f.width))
      ...
      const maxFinisherWidth = Math.max(...this.finishers.slice(0, middle).map(f => f.width))
      for (i = 0; i < this.finishers.length; ++i) {
        if (i < middle) {
